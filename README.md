...
##### Run Instructions
It is advisable to do ```npm i``` in './api' and './frontend' directory before starting the server
To start the API Server, run ```node ./api/index.js``` from the repository's root directory 
To start the frontend Server, run ```npm start``` from the repository's root directory 
